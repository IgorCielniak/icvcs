...
def status():
    ...
    tracked_directories = repo_data.get("directories", {})
    working_directory_files = set()

    # Collect all files in the working directory
    for root, _, files in os.walk('.'):
        if root.startswith(ICVCS_DIR):
            continue
        for file in files:
            working_directory_files.add(os.path.relpath(os.path.join(root, file)))

    untracked_files = working_directory_files - tracked_files
    modified_files = []
    deleted_files = []
    ...
    for file in tracked_files:
        ...
        elif os.path.getmtime(file) > os.path.getctime(file):  # Simple check for modification
            modified_files.append(file)

    # Check for pending commit changes (staged files)
    ...
    for file in untracked_files:
        if not file.startswith(".icvcs"):
            print(f"  {file}")
        if not not file.startswith(".icvcs"):
            no_untracked = True
    if no_untracked == True:
